 ...
 */
class Helper extends ActiveRecord\Model
{
    public static $table_name = 'wheel_dict2parameters';
    ...
    public static function extras(){
        ...
        $result = $model->find_by_sql($query);
        foreach ($result as $row){
            $ids[$row->id]=$row->parameter_id;
            $cleanValues[$row->parameter_id][]=$row->attributes();
        }
        $viewDataTmp = $model->getAliasesFrom($cleanValues);
    ...
    public static function getObjBy($needle,$arr,$by){
        $source = $arr;
