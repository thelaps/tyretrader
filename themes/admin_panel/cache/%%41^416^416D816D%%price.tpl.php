<?php /* Smarty version 2.6.26, created on 2016-01-31 15:11:10
...
/?view=admin_panel&load=price_panel&fnc=prepare">
                                <span class="message">
                                    Перетащите файл прайса сюда..<br /><i>(Открытие произойдет автоматически в 2 этапа)</i><br />
                                    <div class="notificationMessageBlock">
                                        <b>Примечание перед работой*</b>
                                        <ul>
                                            <li>В случае формата когда отдельно указаны ширина, высота<sup>(не всегда)</sup> и диаметр, указывать наименование не нужно, достаточно указать производителя, наименование и так подхватывается по умолчанию на всю строку</li>
                                            <li>Если в строке есть наименование с размерами стандартного форматирования (Ш/В/Р<sup>(не важно какой разделитель)</sup>), то отдельно размеры указывать не стоит, указываем наименование, которое по умолчанию сперва распознает размеры</li>
                                            <li>Наименование работает по обратному алгоритму, сперва размеры, дальше производитель от Я до А и по окончанию модель так же от Я до А с промежуточным списком синонимов по каждой первой букве, если не найден оригинал</li>
                                        </ul>
                                    </div>
                                </span>
                            </div>
...
"><?php echo $this->_tpl_vars['option']->last_update; ?>
: <?php echo $this->_tpl_vars['option']->name; ?>
 - (<?php echo $this->_tpl_vars['option']->city; ?>
)</option>
